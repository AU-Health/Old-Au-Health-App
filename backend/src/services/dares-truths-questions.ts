...
export const getTruthsHistory = async (
  isCurrent,
  isComplete,
  category,
  uuid
  ...
  return dbConnection
    .getTruthsHistory(isCurrent, isComplete, category, uuid)
    .then((result) => {
      let truthArr = []
...
async function getTask(uuid, taskType, category) {
  //currently assuming we are only doing truth types
  ...
  let getNewTask = await dbConnection.getTruthTask(
    category,
    minPointsOfCategory,
    false
  )
  let truthInsertedQueryResult = await dbConnection.assignUserNewTruthTask(
    uuid,
    getNewTask[0].TruthId
  )
