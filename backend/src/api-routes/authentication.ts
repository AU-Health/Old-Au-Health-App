...
router.post('/user_create', authMiddleware.ensureEmailAndPass, (req, res) => {
  ...
  authentication
    .createUserAccount(userEmail, userPass, isAdmin)
...
router.put(
  ...
  authMiddleware.authenticateLogout,
  async (req, res) => {
    res.status(201).json({
      status: 'ok',
...
router.put(
  '/verifyAccount',
  tokenMiddleware.authenticateToken,
  authMiddleware.verifyVerificationCode,
  (req, res) => {
    let uuid = req.user.uuid
    authentication
      .updateAccountPostVerified(uuid)
      .then((verifiedInfo) => {
        res.status(200).json({
          status: 'ok',
